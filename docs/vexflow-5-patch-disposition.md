...
| Immutable draft integration | `0cee84a916f0135d952d83e995089e54a4c34a89` | The combined, clean-install-verified temporary CI dependency; not a substitute for the review units above |
...
| --- | --- |
| `03cfd097` Port vendored OSMD to modern VexFlow | Replaced by neutral migration commits beginning at `82d2ad87`, plus the accepted renderer refinements that make the substitution usable. |
...
| `d64804da` Remove dead notation font overrides | Split into `4266012a` and `821280b6` so archive removal and inactive override removal remain separately reviewable. |
| `66504642` Finalize VexFlow 5 port branch wiring and API seams | Dropped. Its application-specific fork signature is replaced by upstream-neutral build, font, test, and migration documentation. |
...
No application viewer lifecycle, playback, video export, deployment, audio processing, or application asset code is included. The structured diagnostics and browser comparison workflow are intentionally retained because they make renderer changes reviewable; they are not a second production renderer.
