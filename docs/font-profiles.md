...
While the prerequisite VexFlow changes are under review, the draft uses immutable integration commit `0cee84a916f0135d952d83e995089e54a4c34a89`. That integration-only commit adds a package `prepare` hook so a Git install produces the CommonJS and declaration outputs VexFlow's package exports promise, including when npm prepares a source snapshot without `.git` metadata. The OSMD dependency must move to an official released VexFlow version before merge.
