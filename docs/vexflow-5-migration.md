...
The branch contains generic OSMD and VexFlow work. It excludes the originating application's viewer lifecycle, playback, recording, deployment, asset processing, and house-specific UI. Harmony presentation is intentionally separable from the renderer port if maintainers prefer to review that aesthetic policy independently.
